...
from Pix2Pix.dataset import test_loader
...
gen.load_state_dict(torch.load('Pix2Pix/checkpoints/Pix2Pix_Colorize_Anime/gen_500.pth.tar')['state_dict'])
...
'''Kth'''
# num_save = [50, 40, 30, 20, 10, 5]
# for i in num_save:
#     top5_idx = np.argpartition(MAE_PER_AVG, i)[:, :i]
#     top5_idx = top5_idx + (np.array(list(range(num_choice_idx)))*num_noise).reshape(-1, 1)
#     top5_idx = top5_idx.flatten()
#
#     for j in range(len(label_50_50[top5_idx])):
#         save_some_examples(gt=label_50_50[top5_idx][j], test=pre_50_50[top5_idx][j], epoch=j+1, folder=f'save_{i}')
#
#     sio.savemat(f'inference_{i}.mat', {'label_10_31': label_10_31[top5_idx],
#                                        'label_50_50': label_50_50[top5_idx].cpu().numpy(),
#                                        'pre_50_50': pre_50_50[top5_idx].cpu().numpy(),
#                                        'pre_10_31': pre_10_31[top5_idx],
#                                        'noise_10_31': noise_10_31[top5_idx],
#                                        'noise_label_10_31':noise_label_10_31[top5_idx]})
...
'''噪声数据可视化'''
# num_iter = 0
# for (w, x, y, z) in zip(noise_10_31, noise_label_10_31, label_50_50, label_10_31):
#
#     if num_iter==0:
#         for z_dispersion_curve in range(len(z)):
#             plt.plot(np.array(range(len(z[z_dispersion_curve]))), z[z_dispersion_curve])
#             plt.yticks([])
#             plt.xticks([])
#         plt.show()
#
#
#
#     for x_dispersion_curve in range(len(x)):
#         plt.plot(np.array(range(len(x[x_dispersion_curve]))), x[x_dispersion_curve])
#         plt.yticks([])
#         plt.xticks([])
#     plt.show()
#
#     for w_dispersion_curve in range(len(w)):
#         plt.plot(np.array(range(len(w[w_dispersion_curve]))), w[w_dispersion_curve])
#         plt.yticks([])
#         plt.xticks([])
#     plt.show()
#
#     x = torch.from_numpy(x.reshape(-1, 10, 1, 31))
#     y = y.reshape(-1, 50, 50)
#     inference_some_examples(gen, x.to(config.DEVICE), y.to(config.DEVICE), idx=choice_idx[0], epoch=num_iter+1, folder='save')   # N*1*50*50
#     num_iter += 1